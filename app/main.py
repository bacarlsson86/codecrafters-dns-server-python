...
def parse_question_new_method(buf:bytes, offset:int) -> tuple[dict, bytes]:
    ...
    if len(buf) > end_of_question:
        second_question, _ = parse_question_new_method(buf, end_of_question)
        print(f'Second question parsed as {second_question}')
        print(f'End of question {_}')
