...
def parse(buf:bytes) -> dict:
    ...
    request['question'], request['raw_question'] = parse_question_new_method(buf[12:], 0)
    return request
...
def parse_question_new_method(buf:bytes, offset:int) -> tuple[dict, bytes]:
    LABEL_TERMINATOR = b"\x00"
    COMPRESSION_POINTER = b"\xc0"
    parsed_question = {}
    end_of_label = buf[offset:].find(LABEL_TERMINATOR) + 1
    ...
    print(f'Question parsed as {parsed_question}')
    print(f'First buffer {buf[0:end_of_question]}')
    end_of_second_label = buf[end_of_question:].find(LABEL_TERMINATOR) + 1
    second_label = buf[end_of_question:end_of_second_label]
    print(f'Second buffer{second_label}')
