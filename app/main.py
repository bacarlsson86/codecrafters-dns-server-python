...
def parse(buf:bytes) -> dict:
    ...
    request['question'], request['raw_question'] = parse_question(buf[12:], request['headers']['QDCOUNT'])
    return request
...
def parse_question_new_method(buf:bytes, QDCOUNT: int) -> tuple[dict, bytes]:
